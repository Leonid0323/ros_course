...
enum ControlEnum
{
	DUMMY,
	VOYAGER,
	NOTSODUMMY_H, // раскомментировать после добавления нового алгоритма
	nControls
};
...
void laserCallback(const sensor_msgs::LaserScan& msg)
{
	...
	if (controlPtr) // если указатель не нулевой - вызываем функцию текущего
		алгоритма
...
int main(int argc, char** argv)
{
	...
	*/
	ros::Subscriber pose_sub = node.subscribe("/base_pose_ground_truth", 1,
		poseCallback);
	/*
	* Подписываемся на управление переключением алгоритмов
	*/
	ros::Subscriber select_sub = node.subscribe("/selector", 0.1, selectCallback);
	/*
	* Регистрируем функцию обработчик таймера 10Hz
	*/
	ros::Timer timer1 = node.createTimer(ros::Duration(0.001), timerCallback);
	/*
	* Сообщаем, что мы будем публиковать сообщения типа Twist по топику cmd_vel
	* второй параметр - длина очереди
	*/
	cmd_pub = node.advertise<geometry_msgs::Twist>("/cmd_vel", 0.1);
	//инициализация массива доступных алгоритмов
	controls[DUMMY] = new DummyControl();
	controls[VOYAGER] = new VoyagerControl(node.param("min_range", 0.1),
		node.param("max_vel", 0.1),
		node.param("max_omega", 0.1));
	// здесь должны быть инициализированы и другие алгоритмы
	controls[NOTSODUMMY_H] = new NotSoDummy(node.param("min_range", 0.4),
		node.param("max_vel", 0.5),
		node.param("max_omega", 0.5)),
		node.param("min_vel", 0.1);
	//устанавливаем указатель на действующий алгоритм управления на любой (например
	первый) элемент массива
	controlPtr = controls[NOTSODUMMY_H];
	/**
	...
	*/
	ros::spin();
	// сюда мы не попадем, но ради приличия в конце работы удалим динамически выделенную
	памать
